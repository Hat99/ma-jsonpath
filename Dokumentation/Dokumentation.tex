...
Bereits 2007 wurde hierfür, angelehnt an die pfadbasierte \ac{xml}-Abfragesprache \textit{XPath}, die Abfragesprache \textit{JSONPath} vorgeschlagen. Erst 17 Jahre später wurde diese 2024 als \textit{RFC-9535} normiert. 
...
Innerhalb dieser Arbeit wird das System als alleinstehendes Konsolenprogramm entwickelt; eine Weiterentwicklung des Systems für beispielsweise die Einbindung in einen Arbeitsablauf soll jedoch durch klar abgetrennte Komponenten und übersichtliche interne Schnittstellen ermöglicht werden.
...
Für die Erreichung der in Abschnitt \ref{sec:ziel} genannten Ziele werden zunächst die verwendeten Technologien und für das Verständnis dieser Arbeit relevanten Konzepte vorgestellt. 
...
JSONPath-Ausdrücke werden auf valide \ac{json}-Objekte angewendet, um daraus Werte oder Knoten auszuwählen. Das Ergebnis einer JSONPath-Abfrage ist eine Knotenliste, welche im Falle, dass die Abfrage zu einem oder mehreren Knoten im \ac{json}-Objekt führt, diese Knoten enthält.
...
Die regulären Ausdrücke werden in der Reihenfolge, in der sie in der Datei stehen auf den Eingabetext gematcht und entweder das längste gefundene Match oder im Falle von mehreren längsten Matches das in der Datei zuerst geschriebene Match gewählt. \\
Anschließend wird der zum Match gehörende C-Code ausgeführt und zum Beispiel der passende Token zurückgegeben.
...
Ein LR-Parser ist ein sogenannter \textit{Bottom-Up}-Parser für LR-Grammatiken. Dabei wird ausgehend von jeweils einem Blatt des Syntaxbaums versucht, den Eingabetext auf das Startsymbol zu reduzieren, indem die Produktionsregeln der Grammatik rückwärts angewendet werden, was \textit{Reduzieren} genannt wird.
...
Dadurch lassen sich Eingabevalidierung und -auswertung voneinander isoliert behandeln und durch eventuelle optimierungen am Zwischencode auch die Auswertung vereinfachen.
...
Compiler und Interpreter sind dabei an dieser Stelle nur vorübergehende Abschnitte, in welche die Anwendung sich inhaltlich unterteilen lässt. Eine genauere Verteilung der Komponenten und deren Verantwortlichkeitsbereiche erfolgt in Kapitel \ref{cha:loesungskonzept}.
...
Hierbei muss für jeden Parameter dessen Auswirkungen auf Speicherkomplexität und Laufzeit abhängig ergründet werden und sofern möglich müssen Geltungsbereiche und sinnvolle Standardwerte für jeden Parameter definiert werden.
...
Da es im Umfang dieser Arbeit nicht möglich war, die Implementation des JSONPath-Interpreters umzusetzen, wird dieser und mit ihm zusammenhängende Komponenten in diesem Kapitel nicht weiter verfolgt.
...
Die Eingabe wird anhand diverser regulärer Ausdrücke in Tokens unterteilt. Alle Eingaben, die nicht auf einen dieser Tokenausdrücke passen, werden als einzelnes Zeichen dem Parser übergeben. Ausnahme hiervon stellen zum einen Whitespaces dar, da diese im Scanner überlesen werden und zum anderen Zeichen unterhalb von ASCII \textit{\textbackslash x20} (Leerzeichen) und überhalb von ASCII \textit{\textbackslash x7F}, da diese entweder nicht in JSONPath-Ausdrücken auftreten dürfen, oder außerhalb des 7-Bit-ASCII-Bereichs liegen und daher für den Scanner nicht definiert sind.



\subsection{Whitespaces}

Wie bereits erwähnt werden Whitespaces vom Scanner überlesen, um den Parser zu entlasten. Dementsprechend muss der Scanner anderweitig sicherstellen, dass alle Stellen an denen keine Whitespaces erlaubt sind eingehalten werden.
...
Um Whitespaces in Zahlen zu verhindern, werden diese auch weitest möglich über reguläre Ausdrücke gehandhabt. Spezifisch werden Integer hierfür inklusive ihrer eventuellen Vorzeichen definiert, sodass keine Whitespaces zwischen Vorzeichen und Ziffern erlaubt werden. \\
...
Hierbei kann bei Zahlen der Fraktionsteil beziehungsweise Exponent bereits als dessen Zahlenwert dargestellt werden, da die Identifikation über das entsprechende Token erfolgt, also zum Beispiel $1.0$ würde als Tokenfolge einen Integer $1$ gefolgt von einem Fraktionsteil $0$ erkennen. 
...
Der \textit{NORMALIZED}-Zustand ermöglicht die Verarbeitung von der eingeschränkten Grammatik für normalisierte Ausdrücke. Da diese Stardbedingung inklusiv ist, werden die anderen regulären Ausdrücke nach wie vor berücksichtigt, jedoch machen diese keinen Unterschied für die Auswertung, da ihre Tokens in der normalisierten Grammatik nicht akzeptiert werden. 
...
Beispielsweise der String \textit{'a'} kann sowohl als normalisierter String als auch als allgemeiner String interpretiert werden. Damit also die allgemeine Regel auch diesen String erkennen kann, muss diese entweder auch normalisierte Strings akzeptieren, oder der String muss als allgemeiner String tokenisiert werden.
...
Um normalisierte Ausdrücke erkennen zu können, wird ein alternativer Einstieg in die Grammatik benötigt. Hierfür die Startregel \textit{jsonpath} anhand eines vor dem Parsevorgang hinzugefügten Anfangszeichens entscheiden, ob sie die die \textit{jsonpath\_query}-Regel oder die \textit{normalized\_path}-Regel weiterverfolgen soll. Das Steuerzeichen kann dabei entweder ein \textit{j} für allgemeine JSONPath-Ausdrücke oder ein \textit{n} für normalisierte JSONPath-Ausdrücke sein.
...
Die erstellten Syntaxbäume können außerdem mit der Visualisierungskomponente der Bison-Syntaxtree-Hack-Klasse in LaTeX-Form visualisiert werden. Um diese direkt sichtbar zu machen, erzeugt der Controller, wenn die Visualisierung aktiv ist, automatisch die LaTeX-Ausgabe in eine Datei zu überführen und diese zu einer PDF-Datei zu übersetzen. \\
...
Wie an den ersten beiden Beispielen erkennbar wird, erkennt der Parser den normalisierten ersten Ausdruck als solchen und stellt dies auch im Syntaxbaum dar. 
...
Im Folgenden wird die Implementation anhand der in Kapitel \ref{cha:problemanalyse} definierten Anforderungsliste evaluiert, inwiefern die Anforderungen und damit die Ziele der Arbeit erfüllt wurden.
